...
def get_file(file_dir):
    ...
    for file in os.listdir(file_dir + '/house'):
        house.append(file_dir + '/house' + '/' + file)
        label_house.append(0)
    for file in os.listdir(file_dir + '/guitar'):
        guitar.append(file_dir + '/guitar' + '/' + file)
        label_guitar.append(1)


    # 打印出提取图片的情况，检测是否正确提取
    print("There are %d house\n There are %d guitar\n" % (len(house), len(guitar)))
    ...
    image_list = np.hstack((house, guitar))
    label_list = np.hstack((label_house, label_guitar))
    # 利用shuffle，转置、随机打乱
